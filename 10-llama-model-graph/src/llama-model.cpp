// llama-model.cpp - 05 章「建 llama_model + 加载权重」。load_model 5 步：①llama_file 打开一次→②llama_mmap
// 映射→③gguf_load 解析（先拿 n_tensors）→④ggml_init 建池(no_alloc)→⑤逐 tensor new+set_name+data=mmap 基址+offset 零拷贝挂载。
...
namespace llama
{
    ...
    // ---- 析构 ---- 池子里只放 ggml_tensor 结构，释放即可；mmap 是成员，由它自己的析构自动 munmap（RAII）。
    ...
    // ---- 加载入口 ---- 用局部变量做完再一次性写进 llm：中途失败时 llm 保持全空安全态，析构不双释放。
    // 文件只开一次，同一份 llama_file 既喂 gguf_load 又给 llama_mmap。
    bool load_model(const std::string &path, llama_model &llm, std::string &err)
    {
        ...
        // ---- 06 章：从 KV 解析超参数 ---- 读 gguf_context.kv 填 llm.hparams，供语义组装/前向用。
        ...
        // ---- 4. ggml_init：建池（大小按 tensor 个数动态算）---- no_alloc=true 池子只装结构，data 留空由 mmap 填。
        // 每 tensor 占 ggml_object 头(32B)+align16(tensor 结构 160B)=192B；首个对象头再加 32B；留 1KB 余量。
        ...
        // ---- 5. 逐个 tensor：实例化 + 定名 + 零拷贝挂数据 ----
        // data = mmap基址 + gguf_context.offset + ti.offset，直接指向文件里那块权重字节。
        ...
        // ---- 全部成功，一次性提交到 llm ---- mapping 用 std::move 转移给 llm.mmap。
        llm.ctx = ctx;
    ...
    // ---- 06 章：模型语义组装 ---- 根张量 token_embd/output_norm/output；层张量 blk.{il}.attn_*/ffn_*。
    // 建立 name->tensor 查找表按名逐个挂接，缺一即报错。
    bool assemble_model(const llama_model &llm, Model &model, std::string &err)
    {
        ...
        // ---- 根张量 ---- output.weight 可选（权重绑定：lm_head 复用 token_embd，见 src/models/llama.cpp）。
        // output 缺失时 model.output 与 model.token_embd 指向同一块。
