// ggml-kernel.cpp - 分发/组合层实现：把「按 op 调算子」的 switch 搬到这里，
// 只依赖 ggml_tensor 元数据（op/src/ne/op_params/data），拆数组喂给底层 kernel:: 裸 float* 算子。
...
namespace ggml_kernel
{
    ...
    void compute_node(ggml::ggml_tensor *node)
    {
        ...
        {
            // a:[R,n_in] 逐行 × b:[n_in,n_out] -> out:[R,n_out]。按 a 的行循环喂 kernel::matmul，
            // b 每行由 dequant_row 反量化（F32 即 memcpy），故「激活×激活」打分也复用它。
        ...
        {
            // 旋转：src[0]=激活[n_embd,N]（每行一个 token）、src[1]=positions[N]。逐行逐头旋转前 n_rot 维。
        ...
        {
            // a=token_embd:[n_embd,n_vocab]（可能 F16!）、b=token ids 1D，输出逐 token 反量化对应行。
            // 不能用 memcpy——token_embd 是 F16（mmap），得走 dequant_row 转 F32。
            ggml::ggml_tensor *a = node->src[0];
            ggml::ggml_tensor *b = node->src[1];
