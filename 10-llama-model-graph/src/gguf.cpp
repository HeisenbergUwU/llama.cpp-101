// gguf.cpp - GGUF 裸解析器实现：读取对照上游 gguf_init_from_file、校验对照 llama_tensor_weight。
// 教程省去端序转换/多分片/mmap，只讲「怎么读 + 怎么查」。
...
namespace gguf
{
    ...
    // 游标式顺序读取辅助（与上游 static helper 一致）：调 llama_file::read_at 读一个小端序元素，并推进游标 pos。
    template <typename T>
    ...
    bool gguf_load(const llama::llama_file &file, gguf_context &info, std::string &err)
    {
        ...
        // ---- 5. 数据一致性 check：offs = data_offset + tensor_offset 要求 offs+nbytes <= file_size ----
