...
namespace ggml
{
    ...
    // visited 以 tensor 指针为 key 去重——指针唯一，不会像 name 那样因重名误判。
    // 叶子（op==NONE 的常量/输入）进 leafs；算子（op 非 NONE）后序进 nodes。
    ...
    // nodes 已经是「src 先于消费者」的拓扑序（见 ggml_visit），所以按序调用时
    // 每个节点的 src[] 数据必然已就绪（或本就是叶子权重）。
    // 具体「op → 算子实现」的映射收敛在 ggml_kernel::compute_node，
    // 图设施不包含任何算子实现。
