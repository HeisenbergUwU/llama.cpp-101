// llama-hparams.cpp - 06 章「超参数 HParams」：遍历 gguf_context.kv 按 key 填进 HParams。
// 键名="<arch>.<subkey>"（llama）；只处理 tinybrainbot 字段，缺关键字段即报错。
...
namespace llama
{
    namespace
    {
        ...
        // 读一个非数组标量数值 KV（=uint32/float）：gguf.cpp 把标量存成十进制字符串，strtoul/strtof 还原。
