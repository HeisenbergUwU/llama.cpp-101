// llama-vocab.h - 07 章「词表 Vocab」：从 GGUF tokenizer.ggml.* KV 建词表，提供 id<->文本 编解码。
// 教学取舍：上游用 pimpl + 按 model 派发几十种；本迷你版扁平 struct、只支持 tinybrainbot（SPM 型、无 merges）。
...
namespace llama
{
    ...
    struct Vocab
    {
        ...
        // ---- 接口 ---- 从 GGUF KV 建词表（tokens/scores/token_type/特殊 id）。成功 true，失败 false。
