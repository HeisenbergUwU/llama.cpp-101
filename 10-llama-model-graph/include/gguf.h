// gguf.h - GGUF 裸解析器（01 章）：只解析元数据（header/KV/tensor 信息）+ 一致性校验，不 mmap 权重。
// 本机 64 位小端、GGUF 固定小端直接 memcpy；04/05 复用本文件经 llama_file 解析。命名对齐上游 gguf_context/gguf_tensor_info/gguf_kv。
...
namespace gguf
{
    ...
    // ---- 每种张量类型的形状参数（对应 ggml_type_traits 表） ----
    // 上游有 blck_size 与 type_size 两字段；未量化类型 blck_size=1，故 type_size=每元素字节数。
    struct type_trait
    {
        std::string name;
    ...
    // 张量「描述」（只有元数据；对应上游 gguf_tensor_info，上游内嵌 ggml_tensor 作载体，
    // 这里教程不依赖 ggml，改用等价的 ggml-free 字段）
    ...
    // ---- 解析入口 ---- 传入上层已打开的 llama_file（文件只开一次，既喂 gguf_load 又给 mmap）。
    // 成功返回 true 并填进 info；失败返回 false（out 记错误信息）。
