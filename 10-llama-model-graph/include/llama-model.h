// llama-model.h - 05 章「建 llama_model + 加载权重」接口。撮合 01 gguf（解析）/03 ggml（池子建 tensor）
// /04 llama-io（mmap 零拷贝）。只到持有 110 个 ggml_tensor + mmap 映射，不建图/执行/拷权重。
...
namespace llama
{
    ...
    // 加载入口：llama_file 打开一次 -> llama_mmap 映射 -> gguf_load 复用该 file
    // -> 每 tensor: ggml_new_tensor+set_name + data=mmap.addr()+offset。成功 true 填 llm，失败 false。
    ...
    // 按 tensor 名把平铺列表重组进 model 语义结构；期望层数取 n_layer（校验 layers.size()==n_layer）。
    // 根张量缺失或任一层缺权重即报错。成功 true 填好 model；失败 false。
