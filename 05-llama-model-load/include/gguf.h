...
#include "llama-io.h" // 04/05 章：llama_file（05 章让 gguf_load 复用上层打开的文件，不再自己开）
...
namespace gguf
{
    ...
    // ---- 解析入口 ----
    // 传入已由上层(load_model)打开的 llama_file，只解析、不再自己开文件
    // （05 章线性化：文件只打开一次，同一份 file 既喂给 gguf_load 又给 mmap）。
