// gguf.h - GGUF 裸解析器（load+check）：只解析元数据并一致性校验，不 mmap 权重；
// 本机小端/GGUF 固定小端直接 memcpy 读；04/05 章复用（走 llama_file）。对齐 gguf.h/gguf.cpp/llama-model-loader.h。
...
namespace gguf
{
    ...
    // 上游每行有 blck_size/type_size 两字段；未量化 blck_size=1，故 type_size=每元素字节数。
    struct type_trait
    {
        std::string name;
    ...
    // 一个张量的「描述」（只有元数据，没有数据；对应上游 gguf_tensor_info。上游
    // 内嵌 ggml_tensor 做载体，这里教程不依赖 ggml，改用等价的 ggml-free 字段）
    ...
    // ---- 解析入口 ----
    // 传入上层已打开的 llama_file，只解析不再自己开；成功 true 填 info，失败 false（out 记错误）。
