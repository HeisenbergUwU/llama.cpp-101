// ggml.h - 迷你 ggml：03 章加载层接口，在 02 章类型层上补池子分配 API 的声明，
// 命名/字段对齐上游 ggml.h/ggml.c，全放 `namespace ggml`；tinybrainbot 只用 F32=0/F16=1。
...
namespace ggml
{
    ...
    // ---- tensor ----
    // 一个 n 维张量「结构 + 数据指针」，池子核心对象；字段对齐上游 ggml_tensor（暂不引入建图/后端字段）。
    struct ggml_tensor
    {
    ...
    // ---- context ----
    // 仅声明（forward）：ggml_context 是内部实现，字段藏 ggml.cpp，外部拿不透明句柄。
    struct ggml_context;
    ...
    // ==== 03 章·加载层函数接口 ====
    // 在 02 章类型层之上声明「往池子实例化张量」的 API；实现都在 ggml.cpp，对齐上游 ggml.c。
