...
// build: 从 tokenizer.ggml.* 建表；detokenize: SPM 文本直接输出 + byte token 还原；tokenize: SPM 无 merges 逐 token 最长前缀匹配。tinybrainbot 实测（SPM 型 n_vocab=32000）：NORMAL=31731/BYTE=256/USER_DEFINED=9/CONTROL=3/UNKNOWN=1；空格用 ▁(U+2581)；bos=0,eos=7,unk=3,pad=2。
...
namespace llama
{
    ...
    std::vector<int32_t> Vocab::tokenize(const std::string &text, bool add_special) const
    {
        ...
        // 无 merges：逐 token 最长前缀匹配（token_to_id 命中即取）。匹配不到（词表外字符）
        // 按 UTF-8 逐字节编码成 <0xXX> byte token（无损兜底，对齐上游 resegment 的 byte_to_token）；连 byte token 都没有才回退 unk。
        for (size_t i = 0; i < spm_text.size();)
        {
            ...
            // 无命中：取当前 UTF-8 字符（按首字节高位前缀判字节数），逐字节编码 byte token：
            // UTF-8 变长 1~4 字节——0xxxxxxx=1(ASCII) / 110xxxxx=2 / 1110xxxx=3 / 11110xxx=4。
            size_t clen = 1;
            ...
            // 字节级替代写法：逐字节 byte_to_token 直接兜底（不按字符推进）；遇多字节会切碎，故用上面的「按字符推进」版。
        }
