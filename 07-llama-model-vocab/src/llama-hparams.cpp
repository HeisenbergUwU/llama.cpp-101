...
// parse_hparams：遍历 gguf_context.kv 按 key 填进 HParams，键名="<arch>.<subkey>"（arch=llama）；只处理 tinybrainbot 需要的字段，缺关键字段即报错（读者可在此扩展）。
...
namespace llama
{
    namespace
    {
        ...
        // 读一个非数组标量数值型 KV（uint32 或 float）。05 的 gguf.cpp 把标量存成十进制字符串，故用 strtoul/strtof 还原。
