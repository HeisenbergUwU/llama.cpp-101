...
// 把 fopen/fseek/fread/fclose/mmap/munmap 藏起来，对外只暴露「文件」「映射」两对象，上层不再出现裸系统调用；llama_file 对齐 llama-mmap.cpp 的 llama_file::impl、llama_mmap 对齐 llama_mmap::impl，并去掉 prefetch/numa/madvise 等非原理的优化。
...
namespace llama
{
    ...
    // 把已 mmap 好的映射「移交」给另一 llama_mmap（如从局部临时移入 llama_model.mmap）；接管后源要置空，否则双析构会 munmap 同一地址两次。
