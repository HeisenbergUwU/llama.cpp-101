// llama-model.cpp - 05 章「建 llama_model + 加载权重」实现；load_model 5 步（对齐上游 llama_model_loader）：
// 1.llama_file 开文件 2.llama_mmap 映射 3.gguf_load 复用同一 file 解析(拿 n_tensors) 4.ggml_init 建池(no_alloc=true) 5.每 tensor: ggml_new_tensor+set_name+data=mmap.addr()+(offset+ti.offset)。
...
namespace llama
{
    ...
    // ---- 析构 ----
    // 池子里只放 ggml_tensor 结构，释放即可；mmap 是 llama_mmap 成员，会在本析构执行完后由它自己的析构自动 munmap（RAII）。
    ...
    // 用局部变量先做全部工作再一次性写入 llm——中途失败时 llm 保持「全空」安全初始态；文件只开一次，同一份 file 喂给 gguf_load 又给 llama_mmap。
    bool load_model(const std::string &path, llama_model &llm, std::string &err)
    {
        ...
        // ---- 06 章：从 KV 解析超参数 ----
        // 直接读 gguf_context.kv（只读元数据，不依赖 tensor 实例化），结果存进 llm.hparams 供语义组装/前向用（对齐上游 load_hparams 时机）。
        ...
        // no_alloc=true：data 不占池子留钩子；每 tensor 占对象头 32B+align16(160B)=192B，首对象再 +32B，留 1KB 余量。
        ...
        // 数据段起点=gguf_context.offset（对齐后），故 data=mmap基址+offset+ti.offset，直接指向文件里那块权重字节。
    ...
    // ---- 06 章：模型语义组装 ----
    // 已知 tensor 名（LLM_TN_*，llama-arch.cpp）：根=token_embd/output_norm/output.weight；层=blk.{il}.attn_norm/attn_q/attn_k/attn_v/attn_output/ffn_norm/ffn_gate/ffn_up/ffn_down(后缀 .weight)；先建 name->ggml_tensor* 查找表再按名挂接，缺一即报错。
    bool assemble_model(const llama_model &llm, Model &model, std::string &err)
    {
        ...
        // ---- 根张量 ----
        // output.weight 可选：plain llama 常做权重绑定（lm_head 复用 token_embd，见 src/models/llama.cpp 41-46），output 缺失时与 token_embd 指向同一块。
