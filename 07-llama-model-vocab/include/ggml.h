// ggml.h - 迷你 ggml：03 章加载层接口（在 02 章类型之上补池子分配函数声明，实现见 ggml.cpp）
// 命名/字段对齐上游 ggml.h/ggml.c，全在 namespace ggml；tinybrainbot 只用 F32/F16，编号照抄上游（F32=0/F16=1）
...
namespace ggml
{
    ...
    // ---- tensor ----
    // 一个 n 维张量的「结构 + 数据指针」，是池子核心对象；字段对齐上游 ggml_tensor(ggml.h 680)，暂不引入 op/src[]/buffer 等建图、后端字段（后续章节再补）。
    struct ggml_tensor
    {
    ...
    // ---- context ----
    // 仅前向声明：ggml_context 是内部实现，字段藏 ggml.cpp，外部拿不透明句柄经函数操作。
    struct ggml_context;
    ...
    // ==== 03 章·加载层函数接口 ====
    // 在类型层之上声明「往池子里实例化张量」的 API，实现都在 ggml.cpp，对齐上游 ggml/src/ggml.c。
