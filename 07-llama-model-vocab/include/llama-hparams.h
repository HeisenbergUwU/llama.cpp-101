// llama-hparams.h - 06 章「超参数 HParams」接口
// 从 GGUF KV 读「决定架构的超参数」并算派生维度；只做 tinybrainbot(plain llama) 需要的最小字段集——不抄上游上百字段，只留 ~10 个 + 派生 getter；对应上游 load_hparams / llama-hparams.cpp / LLM_KV_* 键名(llama-arch.cpp)。
