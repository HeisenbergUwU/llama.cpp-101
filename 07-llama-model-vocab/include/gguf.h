// gguf.h - GGUF 文件裸解析器接口（load + check，01 章）
// 只解析元数据（header/KV/tensor info）并校验，不依赖 ggml、不 mmap；本机 64 位小端直接 memcpy 读；04/05 复用（gguf_load 经 llama_file 只解析，不再自己开文件）。对应上游 gguf.h / gguf.cpp::gguf_init_from_file / llama-model-loader.h。
...
namespace gguf
{
    ...
    // 上游每行有 blck_size / type_size 两字段；未量化类型 blck_size 恒为 1，故 type_size=每元素字节数
    struct type_trait
    {
        std::string name;
    ...
    // 一个张量的「描述」（只有元数据，无数据；对应上游 ggml/src/gguf.cpp 的 gguf_tensor_info）。
    // 上游内嵌 ggml_tensor 作载体，这里因教程不依赖 ggml，改用等价的 ggml-free 字段。
    ...
    // ---- 解析入口 ----
    // 传入已由上层打开的 llama_file，只解析（04/05 线性化：文件只开一次，同一份喂给 gguf_load 又给 mmap）。成功 true 填好 info；失败 false（err 记错误信息）。
