...
namespace ggml
{
    ...
    // ---- 建池 ----
    // ggml_context 句柄单独 malloc（**不占池子**，只是指向池子的管理器）；params.mem_buffer 为 NULL 时内部 calloc 自分配池子。
    ...
    // ---- 池子切块：末尾追加一块，返回对象头（对齐上游 ggml_new_object）----
    // 布局 [ ggml_object 头 ][ 对齐后的载荷 size ]，obj->offs 指向载荷起点；下一个对象接在载荷之后。
