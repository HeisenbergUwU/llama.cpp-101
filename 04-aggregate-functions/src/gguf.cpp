// gguf.cpp - GGUF 裸解析器实现（load + check）。读取对照上游 gguf_init_from_file，
// 校验对照 llama_tensor_weight；省去端序转换/多分片/mmap，只讲「怎么读 + 怎么查」。
...
namespace gguf
{
    ...
    // 游标式顺序读取辅助（同上游 gguf.cpp 的 static helper）：小端序读一个元素。
    // 直接调 llama_file::read_at，这里只补「游标推进」。
    template <typename T>
    ...
    bool gguf_load(const llama::llama_file &file, gguf_context &info, std::string &err)
    {
        ...
        // ---- 5. 数据一致性 check：每张量数据都在文件界内（offs=data_offset+tensor_offset，要求 offs+nbytes<=file_size）----
