// llama-io.h - 04 章「文件 IO 封装层」接口：把 open/fstat/mmap/close 等裸系统调用
// 封装成 llama_file（FILE* 顺序读元数据 + 暴露 fd 供 mmap）与 llama_mmap（映射整个文件）两个类型。
...
namespace llama
{
    ...
    // 两种能力:① read_at/read_le 顺序缓冲读元数据;② 暴露裸 fd 供 mmap 零拷贝
    struct llama_file
    {
    ...
    // ---- 内存映射封装:mmap + munmap（对齐上游 llama_mmap）----
    // 简化掉 prefetch/numa/madvise 等调优，只留核心:mmap 整个文件(PROT_READ | MAP_PRIVATE)、析构 munmap。
    struct llama_mmap
    {
