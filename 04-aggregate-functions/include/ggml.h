// ggml.h - 迷你 ggml：在 02 章类型层（ggml_type/ggml_tensor/ggml_context）之上补充
// 池子分配 API 声明，实现放 ggml.cpp；命名对齐上游 ggml/include/ggml.h，全部放 namespace ggml。
...
namespace ggml
{
    ...
    // ---- tensor ----
    // n 维张量的「结构 + 数据指针」，池子核心对象；暂不引入 op/src[]/buffer 等建图字段。
    struct ggml_tensor
    {
    ...
    // ---- context ----
    // 仅 forward declaration：不透明句柄，字段藏在 ggml.cpp，外部只能经函数接口操作。
    struct ggml_context;
    ...
    // ==== 03 章·加载层函数接口：声明「往池子里实例化张量」的 API，实现都在 ggml.cpp ====
