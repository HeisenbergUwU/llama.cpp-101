...
做法（最小化结构版）：
- 04 章 = 把裸系统调用（`open`/`fstat`/`mmap`/`munmap`/`close`）封装成 `llama_file` + `llama_mmap` 两个小类型，让上层逻辑不掺和 OS 细节。
- `llama_file`：打开文件、`fstat` 拿大小、析构 `close`。
- `llama_mmap`：把整个文件 `PROT_READ | MAP_SHARED` 映射进地址空间、析构 `munmap`。
- 教程去掉上游的 `prefetch`/`numa`/`MAP_POPULATE`/`posix_madvise`/pimpl（那些是性能优化与封装技巧，非 mmap 原理）。
