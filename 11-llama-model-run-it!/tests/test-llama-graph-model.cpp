// test-llama-graph-model.cpp - 10 章「整体模型图」手写测试（AGENTS.md：手写 main，退出码非 0=失败）。加载
// tinybrainbot->assemble->build_model_graph->compute 跑出 logits->校验 + 末 token argmax（与 08 直写 "Paris" 对齐）。
...
int main(int argc, char **argv)
{
    ...
    // ---- 加载（load_model 一步完成：tensor + 语义组装，填入 llm.model） ----
    ...
    llama::Model &model = llm.model; // 语义结构挂在 llama_model 上
    ...
    // ---- 池子：no_alloc=false，算子节点 data 进池 ---- 大小按「全部中间张量+节点结构」估的保守上限
    // （不是文件大小）。tinybrainbot 12 层：每层若干 [768,N]/[2048,N]/[256,N] + logits[32000,N]。
