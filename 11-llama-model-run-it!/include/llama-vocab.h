// llama-vocab.h - 07 章「词表 Vocab」接口（最小可跑）：从 GGUF tokenizer.ggml.* KV 建词表。教学取舍：
// 上游 pimpl+按架构派发 SPM/BPE/WPM 几十种；本迷你版扁平 struct、直接读 gguf_context.kv、只支持 SPM（无 merges）。
...
namespace llama
{
    ...
    struct Vocab
    {
        ...
        // ---- 接口 ---- 从 GGUF KV 建词表（tokenizer.ggml.tokens/scores/token_type/特殊 id），
