// ggml.h - 迷你 ggml：数据结构声明 + 加载层函数接口（03 章）。在 02 章类型层上补池子分配 API
// 声明，实现放 ggml.cpp，字段对齐上游 ggml.h/.c，全放 namespace ggml（避开 01 章全局 GGML_TYPE_*）。
...
// 断言（对齐上游 GGML_ASSERT）：条件为假打印并 abort()，用于「建图阶段就非法」的
// 入参校验（如 permute 越界轴）；graph 单测用 fork+waitpid(WIFSIGNALED) 验证 abort。
#include <stdio.h>
#include <stdlib.h>
...
namespace ggml
{
    ...
    // ---- 算子类型 ---- NONE=叶子（权重/输入）；其余为算子节点，对齐上游 GGML_OP_*。
    // 保留「通用+前向必需」算子：embed 查表/注意力 softmax/布局重排都作可组合算子入图，不单列独有算子。
    enum ggml_op
    {
        GGML_OP_NONE = 0,
        GGML_OP_RMS_NORM,
    ...
    // ---- tensor ---- n 维张量的「结构+数据指针」，池子核心对象：既可是权重/输入叶子
    // （op=NONE，data 指向数据）也可是算子节点（op 非 NONE，src[] 记输入、data 存结果），与上游一致。
    struct ggml_tensor
    {
    ...
    // ---- context ----
    // 只前向声明：ggml_context 是内部实现，字段藏在 ggml.cpp，外部拿不透明句柄走函数接口。
    struct ggml_context;
    ...
    // ==== 03 章·加载层函数接口 ====
    // 在 02 章类型层上声明「往池子里实例化张量」的 API，实现都在 ggml.cpp，对齐上游对应函数。
    ...
    // 已用池子字节数（到最后一个对象载荷末尾的高水位）。
    // 对应上游 ggml_used_mem；11 章采样器用它核对「每步重建的池子够不够大」。
    size_t ggml_mem_used(const ggml_context *ctx);

    // 重置池子：清空对象链表但保留 mem_buffer 不释放。供 llm 复用：下一步 build 从池子起点
    // 重新 append 覆盖旧图，避免每步 malloc/free（对齐上游 ggml_gallocr 复用 buffer）。
