// llama-io.h - 04 章「文件 IO 封装层」接口：把裸系统调用（open/mmap/munmap/fread/fseek）封装成
// 「文件/映射」两类型，去 pimpl 直接暴露成员。llama_file 持 FILE*+fd（顺序读兼供 mmap）；llama_mmap 只映射全文件。
...
namespace llama
{
    ...
    // ---- 文件封装:open + fstat + 顺序读 + close ---- 对齐上游 llama_file（llama-mmap.h:16），
    // 去 pimpl 但保留两种能力：① 顺序缓冲读（read_at/read_le，gguf 解析用）② 裸 fd（供 mmap）。
    struct llama_file
    {
    ...
    // ---- 内存映射封装:mmap + munmap ---- 对齐上游 llama_mmap（llama-mmap.h:43），去掉
    // prefetch/numa/madvise 等调优，只留核心：mmap 整个文件（PROT_READ|MAP_PRIVATE）、析构时 munmap。
    struct llama_mmap
    {
