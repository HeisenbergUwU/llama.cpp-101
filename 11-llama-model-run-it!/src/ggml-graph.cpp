...
namespace ggml
{
    ...
    // 递归：把 tensor 依赖链后序登记进 cgraph（共享子图只登记一次）。visited 以指针为 key
    // 去重（指针唯一，不似 name 会重名误判）。叶子（op==NONE）进 leafs，算子（op 非 NONE）后序进 nodes。
    ...
    // ---- 执行：按 nodes 拓扑序逐个节点交给 ggml-kernel 分发执行 ---- nodes 已是「src 先于消费者」的
    // 拓扑序，按序调用时各节点 src[].data 必然就绪（或本就叶子权重）。op→实现的映射收敛在 compute_node。
