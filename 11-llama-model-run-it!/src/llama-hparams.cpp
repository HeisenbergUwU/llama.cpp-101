// llama-hparams.cpp - 06 章「超参数 HParams」实现：parse_hparams 遍历 kv 按 key（"<arch>.<subkey>"，
// arch 对 plain llama="llama"）把值填进 HParams；只处理 tinybrainbot 所需字段，缺关键字段即报错。
...
namespace llama
{
    namespace
    {
        ...
        // 读非数组标量数值 KV：05 的 gguf.cpp 把标量统一存成十进制字符串，故 strtoul/strtof 还原。
