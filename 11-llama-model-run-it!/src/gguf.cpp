// gguf.cpp - GGUF 文件裸解析器实现（load + check）。读取逻辑对照上游 ggml/src/gguf.cpp 的
// gguf_init_from_file，校验对照 llama-model-loader.h 的 llama_tensor_weight。省去端序/分片/mmap。
#include "gguf.h"
...
namespace gguf
{
    ...
    // 游标式顺序读取辅助（对齐上游 gguf.cpp 的 static helper），直接调 llama_file::read_at
    // 读一个小端元素、游标推进 sizeof(T)。
    template <typename T>
    ...
    bool gguf_load(const llama::llama_file &file, gguf_context &info, std::string &err)
    {
        ...
        // ---- 5. 数据一致性 check：每个张量数据都在文件界内 ---- 对应 llama-model-loader.h:
        // offs = data_offset + tensor_offset; 要求 offs + nbytes <= file_size
