// ggml-ops.cpp - 算子构造函数实现（09 章）。每个算子自池子 new 一个算子节点 tensor（对齐上游
// ggml_*_impl），填 op/src/op_params/name；不真正计算，执行在 ggml-kernel 的分发里。
...
namespace ggml
{
    ...
    {
        // a:[n_in, R]（ne={n_in,R}）b:[n_in,n_out] -> out:[n_out, R]。权重 W 存 ne[0]=n_in（行=输出特征、
        // 长 n_in）、ne[1]=n_out（输出特征数），由 dequant_row 逐行反量化；输出每行 = 一个 token（ne[1]=R）。
