// llama-vocab.cpp - 07 章「词表 Vocab」实现：build=从 tokenizer.ggml.* 建表；detokenize=SPM 文本直接输出、
// byte token 还原 1 字节；tokenize=SPM 无 merges、逐 token 最长前缀匹配。tinybrainbot（SPM,n_vocab=32000）。
...
namespace llama
{
    ...
    bool Vocab::build(const gguf::gguf_context &ctx, std::string &err)
    {
        ...
        // token_type 对应上游 llama_vocab_token_type：
    ...
    std::vector<int32_t> Vocab::tokenize(const std::string &text, bool add_special) const
    {
        ...
        // 无 merges：逐 token 最长前缀匹配（token_to_id 命中即取）。匹配不到（词表外字符）按
        // UTF-8 逐字节编码成 <0xXX> byte token 兜底（对齐上游 resegment 的 byte_to_token）。
        for (size_t i = 0; i < spm_text.size();)
        {
            ...
            // 无命中：取当前 UTF-8 字符（变长 1~4 字节，由首字节高位前缀判定：0xxxxxxx=1、
            // 110xxxxx=2、1110xxxx=3、11110xxx=4），逐字节编码 byte token，并推进 clen 防死循环。
            size_t clen = 1;
            ...
            // 注：若不做字符级多字节编码，可退化为「逐单字节 byte_to_token」的简化变体。
        }
