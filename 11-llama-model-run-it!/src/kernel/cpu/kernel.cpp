// kernel/cpu/kernel.cpp - 08 章「算子内核」CPU 实现：纯计算算子（直接循环不建图），与 forward/模型胶水解耦。
// 权重留 mmap data 上，matmul 一次只 dequant 一行进 scratch、F32 累加（整章唯一懂 F16 布局的是 dequant_row）。
...
namespace kernel
{
    ...
    // 不能标 inline（头文件也是普通声明）：双方都标 inline 则不导出符号，test-kernel 会 undefined symbol。
    ...
    // F16 逐元素 fp16_to_fp32、F32 直接 memcpy（4 字节/元素、小端一致）。
    ...
    // ---- RMS 归一化（逐行） ----
    // rms=sqrt(mean(x²)+eps)；out[j]=x[j]/rms·w[j]。w 是一维 [n] F32 weight（attn/ffn/output_norm）。
    ...
    // ---- 矩阵乘（权重在前）---- W 是 [n_in, n_out]（ne[0]=n_in 输入、ne[1]=n_out 输出，行主序）。
    // out[j]=Σ_i x[i]·Wrow[j][i]，Wrow[j] 反量化到 scratch（长 n_in）。
