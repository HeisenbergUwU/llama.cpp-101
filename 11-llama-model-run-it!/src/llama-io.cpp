// llama-io.cpp - 04 章「文件 IO 封装层」实现：把系统调用（fopen/fseek/fread/fclose/mmap/munmap）藏起来，
// 只暴露「文件/映射」两对象，对齐 llama_mmap.cpp 的 impl；去掉 prefetch/numa/madvise 等非原理优化。
...
namespace llama
{
    ...
    // 把已映射好的映射"移交"给另一 llama_mmap（如从局部临时移入 llama_model.mmap）；接管后须将源置空防双 munmap。
