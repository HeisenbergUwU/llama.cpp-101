...
namespace ggml
{
    ...
    // ---- 建池 ---- 对齐 ggml_init：ggml_context 句柄单独 malloc（不占池子只作管理器），mem_buffer 池子
    // 只放对象/tensor，params.mem_buffer 为 NULL 时内部 calloc 一块。
    ...
    // ---- 池子切块：在末尾追加一块，返回对象头 ---- 布局（对齐 ggml_new_object）：[ggml_object 头][载荷
    // size]，obj->offs 指载荷起点、obj->size 是「对齐后的载荷大小」，下个对象接在载荷之后。
                                             ...
                                             ggml_tensor *view_src, size_t view_offs)
    {
        ...
        // 建图字段默认：NONE 叶子 / op_params 清 0 / src 全 NULL（算子 API 再填）
        t->op = GGML_OP_NONE;
        for (int i = 0; i < (int)(GGML_MAX_OP_PARAMS / sizeof(int32_t)); i++)
        {
            t->op_params[i] = 0;
        }
    ...
    // ---- op_params 读写（对齐上游 ggml_set/get_op_params_i32/f32）----
    void ggml_set_op_params_i32(ggml_tensor *a, int slot, int32_t v)
    {
        a->op_params[slot] = v;
    }

    int32_t ggml_get_op_params_i32(const ggml_tensor *a, int slot)
    {
        return a->op_params[slot];
    }

    void ggml_set_op_params_f32(ggml_tensor *a, int slot, float v)
    {
        memcpy(&a->op_params[slot], &v, sizeof(v));
    }

    float ggml_get_op_params_f32(const ggml_tensor *a, int slot)
    {
        float v;
        memcpy(&v, &a->op_params[slot], sizeof(v));
        return v;
    }
    ...
    size_t ggml_mem_used(const ggml_context *ctx)
    {
        if (ctx == NULL || ctx->objects_end == NULL)
        {
            return 0;
        }
    ...
    void ggml_reset(ggml_context *ctx)
    {
        if (ctx == NULL)
        {
            return;
        }
        // 只清空对象链表、保留 mem_buffer（不 free）：让下一次 build 从池子起点
        // 重新 append、覆盖旧数据。mem_size / mem_buffer / no_alloc 保持不变。
        ctx->objects_begin = NULL;
        ctx->objects_end = NULL;
        ctx->n_objects = 0;
    }
