...
namespace ggml
{
    ...
    // ---- 建池 ----
    // 对齐上游 ggml_init：ggml_context 这个「句柄结构」单独 malloc 一块、不占池子，它只是指向池子的管理器，mem_buffer 池子只放对象/tensor；若 params.mem_buffer 为 NULL，内部 calloc 一块作为池子。
    ...
    // ---- 池子切块：在末尾追加一块，返回对象头 ----
    // 布局（对齐上游 ggml_new_object）：[ ggml_object 头 ][ 载荷 size ]，obj->offs 指向载荷起点、obj->size 是对齐后的载荷大小，下一个对象接在载荷之后。
