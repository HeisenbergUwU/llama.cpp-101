...
// 读取对照上游 ggml/src/gguf.cpp 的 gguf_init_from_file，校验对照 src/llama-model-loader.h 的 llama_tensor_weight；教程省去端序转换、多分片、mmap 等，只讲「怎么读 + 怎么查」。
...
namespace gguf
{
    ...
    // 游标式顺序读取辅助（与上游 gguf.cpp 的 static helper 一致写法）：读取直接调 llama_file::read_at，这里只补「游标推进」，小端序读取一个元素。
    template <typename T>
