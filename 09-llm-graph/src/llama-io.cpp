...
// 把 fopen/fseek/fread/fclose/mmap/munmap 藏起来，对外只暴露「文件」「映射」两个对象，上层不再出现裸系统调用。llama_file 对齐上游 llama_mmap.cpp 的 llama_file::impl（FILE* 读元数据、fd=fileno(fp) 供 mmap）；llama_mmap 对齐 llama_mmap::impl（mmap(...)）；去掉 prefetch/numa/madvise 等性能优化（非原理）。
...
namespace llama
{
    ...
    // 把一个已 mmap 好的映射「移交」给另一个 llama_mmap（如从局部临时移入 llama_model.mmap）；接管后要把源置空，否则两者析构会 munmap 同一地址两次。
