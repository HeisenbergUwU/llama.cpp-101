...
// 教程 01 章自包含实现：只解析 GGUF 元数据（header、KV、tensor info）并一致性校验，不依赖 ggml、不 mmap 权重；本机 64 位小端、GGUF 固定小端，直接 memcpy 读。04/05 章复用本文件：gguf_load 经 llama_file 只解析不再自己开文件。命名对齐上游（gguf_context/gguf_tensor_info/gguf_kv）。
...
namespace gguf
{
    ...
    // 上游每行有 blck_size/type_size 两字段；未量化类型 blck_size 恒为 1，故 type_size=每元素字节数。保留两字段与上游对齐。
    struct type_trait
    {
        std::string name;
    ...
    // gguf_tensor_info。上游内嵌 ggml_tensor 作载体，这里因教程不依赖 ggml 改用等价的 ggml-free 字段）
    ...
    // ---- 解析入口 ----
    // 传入已由上层打开的 llama_file，只解析不自己开文件（04/05 线性化：文件只开一次，同一份 file 既喂 gguf_load 又给 mmap）；成功填 info 返回 true，失败返回 false（out 记错误）。
