...
// 从 GGUF 的 KV 元数据读出决定架构的超参数并算派生维度；只做 tinybrainbot（plain llama）的最小字段集（~10 个+派生 getter），不抄上游上百字段的 llama_hparams。对应上游：KV 读取看 llama-model.cpp 的 load_hparams、派生 getter 看 llama-hparams.cpp、KV 键名看 llama-arch.cpp 的 LLM_KV_*。
