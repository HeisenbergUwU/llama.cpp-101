...
// 从 GGUF 的 tokenizer.ggml.* KV 建词表，提供 id<->文本编解码。教学取舍（对照上游 llama-vocab.h/.cpp）：上游用 pimpl+unique_ptr 藏实现、从 llama_model_loader+LLM_KV 读、按 model 派发 SPM/BPE/WPM 等几十种；本迷你版扁平 struct、直接读 gguf_context.kv、只支持 tinybrainbot（model="llama" 即 SPM，无 merges），故不引入 llama_vocab_pre_type 全表、normalizer_options 等，最小集够用。
...
namespace llama
{
    ...
    struct Vocab
    {
        ...
        // ---- 接口 ----
        // 从 GGUF KV 建词表（tokenizer.ggml.tokens/scores/token_type/特殊 id）；成功 true 填好本对象，失败 false（err 写原因）。
