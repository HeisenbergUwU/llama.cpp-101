...
namespace llama
{
    ...
    // 按 tensor 名把 llm.tensors 平铺列表重组进 model 的语义结构。
    // 期望层数取 llm.hparams.n_layer（校验 model.layers.size() == n_layer）。
    // 根张量（token_embd/output_norm/output）缺失或任一层缺任一个权重即报错。
    // 成功 true 填好 model；失败 false（err 写原因）。
