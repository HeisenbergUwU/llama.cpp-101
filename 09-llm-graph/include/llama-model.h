...
// 只声明结构与函数，实现留 src/llama-model.cpp。撮合前几章：01 gguf 解析（每 tensor 的 offset/type/ne/name）、03 ggml 募池实例化 ggml_tensor（no_alloc=true，data 留钩子）、04 llama-io 映射整个文件让 data 零拷贝指向文件。只到「持有 110 个 ggml_tensor + mmap 映射」，不建图、不执行、不做 hparams/vocab、不拷权重，故不引入上游 llama_model_params。
...
namespace llama
{
    ...
    // 流程：llama_file 打开一次 -> llama_mmap 映射（同一 file）-> gguf::gguf_load 复用该 file -> 每 tensor: ggml_new_tensor + set_name + data = mmap.addr()+info.offset+ti.offset。成功 true 填好 llm，失败 false（err 写原因）。
    ...
    // 按 tensor 名把 llm.tensors 平铺列表重组进 model 的语义结构（期望层数取 llm.hparams.n_layer，校验 model.layers.size()==n_layer；根张量或缺任一层权重即报错）。成功 true 填好 model，失败 false（err 写原因）。
