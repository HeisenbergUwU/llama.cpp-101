...
// 把裸系统调用（open/fstat/mmap/munmap/close/fread/fseek）封装成两个小类型，上层（gguf/llama_model）不直接碰系统调用、只看「文件」「映射」两个概念；对齐上游 llama-mmap.h 的 llama_file/llama_mmap，但去 pimpl 直接暴露成员方便读者看穿每一层。llama_file 内部持 FILE*（缓冲顺序读元数据）+ 暴露底层 fd（供 mmap 零拷贝）；llama_mmap 只负责把整个文件映射进地址空间。
...
namespace llama
{
    ...
    // 对齐上游 struct llama_file（llama-mmap.h:16）。教程去 pimpl，保留两种能力：① 顺序缓冲读（read_at/read_le，gguf 解析元数据用）；② 裸 fd（供 mmap 零拷贝）。
    struct llama_file
    {
    ...
    // ---- 内存映射封装:mmap + munmap ----
    // 对齐上游 struct llama_mmap（llama-mmap.h:43）。教程简化掉 prefetch/numa/MAP_POPULATE/madvise 等性能调优，只留核心：mmap 整个文件（PROT_READ|MAP_PRIVATE）、拿地址、析构时 munmap。
    struct llama_mmap
    {
