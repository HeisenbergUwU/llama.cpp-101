// llama-io.cpp - 04 章「文件 IO 封装层」实现。把 fopen/fread/mmap/munmap 藏起来，对外只露「文件」「映射」两对象。
// llama_file 对齐 llama_file::impl（FILE* 读元数据、fd=fileno(fp) 供 mmap），llama_mmap 对齐 impl；去掉 prefetch/numa/madvise。
...
namespace llama
{
    ...
    // ---- llama_mmap:移动构造/移动赋值：把映射「移交」给另一 llama_mmap ----
    // 接管后须把源 addr 置空，否则双析构会对同一地址 munmap 两次。
