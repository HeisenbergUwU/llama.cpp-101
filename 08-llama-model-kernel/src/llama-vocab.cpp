// llama-vocab.cpp - 07 章「词表 Vocab」实现。build 从 tokenizer.ggml.* 建表；detokenize
// SPM 文本直出 / byte token 还原 1 字节；tokenize 无 merges 最长前缀匹配。SPM 空格用 U+2581，特殊 id bos=0 eos=7 unk=3 pad=2。
...
namespace llama
{
    ...
    bool Vocab::build(const gguf::gguf_context &ctx, std::string &err)
    {
        ...
        // token_type 对应上游 llama_vocab_token_type：0 UNDEFINED/1 NORMAL/2 UNKNOWN/
    ...
    std::vector<int32_t> Vocab::tokenize(const std::string &text, bool add_special) const
    {
        ...
        // 无 merges：逐 token 最长前缀匹配（token_to_id 命中即取）。匹配不到（词表外）
        // 按 UTF-8 逐字节编码成 <0xXX> byte token（对齐 resegment byte_to_token），仍无才回退 unk。
        for (size_t i = 0; i < spm_text.size();)
        {
            ...
            // 变长编码 1~4 字节：0xxxx...=1、110x=2、1110x=3、11110x=4（如 é/中/emoji）。
            size_t clen = 1;
