...
namespace
{
    ...
    // 把一段 flat 激活行包成 1 维 F32 tensor 视图（ne[0]=长度，data 指到数组）。
    ggml::ggml_tensor row(float *data, int64_t ne0)
    {
        ggml::ggml_tensor t{};
        t.type = ggml::GGML_TYPE_F32;
        t.ne[0] = ne0;
        t.nb[0] = sizeof(float);
        t.nb[1] = sizeof(float) * ne0;
        t.data = data;
        return t;
    }
}
...
int main()
{
    ...
    {
        float xd[3] = { 2.0f, 2.0f, 2.0f };
        float wdata[3] = { 1.0f, 1.0f, 1.0f };
        float od[3];
        ggml::ggml_tensor W;
        ...
        W.ne[0] = 3;
        W.data = wdata;
        bool ok = kernel::rms_norm(row(xd, 3), &W, 1e-5f, row(od, 3));
        CHECK(ok, "rms_norm 返回 true（尺寸匹配）");
        CHECK(approx(od[0], 1.0f, 1e-3f) && approx(od[1], 1.0f, 1e-3f) &&
              approx(od[2], 1.0f, 1e-3f), "rms_norm 常数向量(W=1) 归一化为 1");

        // 负向：out 长度与 W 不符 -> 返回 false
        float od2[2];
        CHECK(!kernel::rms_norm(row(xd, 3), &W, 1e-5f, row(od2, 2)), "rms_norm out 尺寸不符返回 false");
    }
    ...
    // --- matmul / dequant_row：本地 F32 权重 [n_in=2, n_out=3] ---
    {
        ...
        float xd[2] = { 1.0f, 1.0f };
        float od[3];
        std::vector<float> scratch(2);
        bool ok = kernel::matmul(row(xd, 2), &W, scratch.data(), row(od, 3));
        CHECK(ok, "matmul 返回 true（尺寸匹配）");
        CHECK(approx(od[0], 3.0f) && approx(od[1], 7.0f) && approx(od[2], 11.0f),
              "matmul(F32) out[j]=Σ x[i]*W[j][i]");

        // 负向：x 长度 2 但 W 是 [2,3]，x 变成 3 -> W->ne[0]!=x->ne[0] -> false
        float xd_bad[3];
        CHECK(!kernel::matmul(row(xd_bad, 3), &W, scratch.data(), row(od, 3)),
              "matmul x 尺寸不符返回 false");
