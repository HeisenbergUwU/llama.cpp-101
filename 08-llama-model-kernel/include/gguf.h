// gguf.h - GGUF 文件裸解析器（load + check）。只解析元数据（header/KV/tensor info）并校验，不依赖 ggml、不 mmap 权重；
// 本机 64 位小端、GGUF 固定小端直接 memcpy；04/05 复用经 llama_file 只解析。对齐上游 ggml/include/gguf.h 与 gguf.cpp。
...
namespace gguf
{
    ...
    // 上游每行有 blck_size / type_size 两字段；未量化类型 blck_size=1，故 type_size=每元素字节数。
    struct type_trait
    {
        std::string name;
    ...
    // 一个张量「描述」（只有元数据无数据；对应上游 gguf_tensor_info。上游内嵌
    // ggml_tensor 作载体，教程不依赖 ggml，改等价 ggml-free 字段）。
    ...
    // ---- 解析入口：传入上层已打开的 llama_file 只解析（文件只开一次，同份喂 gguf_load
    // 又 mmap）；成功 true 填 info，失败 false（err 记原因）。----
