// llama-io.h - 04 章「文件 IO 封装层」接口。把裸系统调用封装成两类型：llama_file 持 FILE*
// 顺序读并暴露 fd（供 mmap），llama_mmap 映射整个文件；无 pimpl 直接暴露，对齐上游 llama-mmap.h。
...
namespace llama
{
    ...
    // ---- 文件封装:open+fstat+顺序读+close（对齐上游 llama_file:16）----
    // 保留两能力：①顺序缓冲读(read_at/read_le)供 gguf 解析元数据；②裸 fd 供 mmap 零拷贝。
    struct llama_file
    {
    ...
    // ---- 内存映射封装:mmap+munmap（对齐上游 llama_mmap:43）----
    // 简化去 prefetch/numa/madvise，只留核心：mmap 整个文件(PROT_READ|MAP_PRIVATE)、拿地址、析构 munmap。
    struct llama_mmap
    {
