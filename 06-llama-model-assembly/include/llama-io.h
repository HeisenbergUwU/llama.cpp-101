// llama-io.h - 04 章「文件 IO 封装层」接口：把裸系统调用（open/fstat/mmap/munmap/close/fread/fseek）封装成两个小类型，
// 上层只见「文件」「映射」。对齐上游 llama-mmap.h 的 llama_file/llama_mmap，去掉 pimpl 直接暴露成员。llama_file 持 FILE* 读元数据并暴露 fd（供 mmap）；llama_mmap 映射整个文件。
...
namespace llama
{
    ...
    // ---- 文件封装:open + fstat + 顺序读 + close ----（对齐 llama-mmap.h:16，去 pimpl）
    // 两种能力：① read_at/read_le 顺序缓冲读（gguf 解析元数据）② 裸 fd 供 mmap 零拷贝。
    struct llama_file
    {
    ...
    // ---- 内存映射封装:mmap + munmap ----（对齐 llama-mmap.h:43）
    // 简化掉 prefetch/numa/madvise 等性能调优，只留核心：mmap 整个文件(PROT_READ|MAP_PRIVATE)、拿地址、析构时 munmap。
    struct llama_mmap
    {
