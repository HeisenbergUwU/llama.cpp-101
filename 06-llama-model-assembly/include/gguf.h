// gguf.h - GGUF 文件裸解析器（load + check）：只解析元数据（header、KV、tensor 信息）并校验，不依赖 ggml、不 mmap。
// 本机 64 位小端、GGUF 固定小端，直接 memcpy 读；04/05 复用本文件（gguf_load 经 llama_file 只解析）。对照上游：布局 ggml/include/gguf.h、读取 ggml/src/gguf.cpp、越界校验 llama-model-loader.h；命名对齐 gguf_context/gguf_tensor_info/gguf_kv。
...
namespace gguf
{
    ...
    // 上游每行有 blck_size/type_size 两字段（ggml.h 2890-2898）；未量化类型 blck_size 恒为 1，故保留两字段与上游对齐。
    struct type_trait
    {
        std::string name;
    ...
    // 一个张量的「描述」（只有元数据，对应上游 ggml/src/gguf.cpp 的 gguf_tensor_info，
    // 上游内嵌 ggml_tensor 作载体，这里教程不依赖 ggml 改用等价的 ggml-free 字段）
    ...
    // ---- 解析入口 ----：传入已由上层打开的 llama_file，只解析、不再自己开文件
    // （04/05 线性化：文件只开一次，同一份 file 既喂 gguf_load 又给 mmap）。成功 true 填 info；失败 false（err 记原因）。
