// llama-model.h - 05 章「建 llama_model 聚合对象 + 加载权重」接口（最小化）：只声明结构与函数，实现在 src/llama-model.cpp。
// 撮合前几章：01 gguf 解析给 offset/type/ne/name → 03 ggml 池实例化 tensor（no_alloc）→ 04 llama_mmap 映射文件零拷贝指向 data。范围到「持 110 tensor + mmap」，不建图/不执行/不做 hparams/vocab/不拷权重。
...
namespace llama
{
    ...
    // 加载入口：llama_file 打开一次 -> llama_mmap 映射（同一 file）-> gguf::gguf_load 复用
    // -> 每 tensor: ggml_new_tensor + set_name + data = mmap.addr() + info.offset + ti.offset。成功 true 填好 llm；失败 false（err 写原因）。
    ...
    // 按 tensor 名把 llm.tensors 重组进 model 语义结构。期望层数取 llm.hparams.n_layer（校验 layers.size()==n_layer）。
    // 根张量缺失或任一层缺任一权重即报错。成功 true 填好 model；失败 false（err 写原因）。
