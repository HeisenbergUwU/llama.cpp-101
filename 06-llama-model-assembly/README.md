...
- `Layer` / `Model`：字段名对齐上游 `llama_layer`，每层一个 9 权重 bag（`attn_norm`/`wq`/`wk`/`wv`/`wo`/`ffn_norm`/`gate`/`up`/`down`）+ 根张量（`token_embd`/`output_norm`/`output`）
- `output` 缺失时绑定 `token_embd`（权重绑定 / tied lm_head）
- `assemble_model`：按 tensor 名把 `llm.tensors` 重组进 `Model`
- 仍不建图、不执行、不拷权重；Vocab 移出为 07 章
...
| `model-graph.md` | 模型结构图（Mermaid + ASCII：架构 / 类图 / GQA / FFN） |
