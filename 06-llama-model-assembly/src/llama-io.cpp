// llama-io.cpp - 04 章「文件 IO 封装层」实现：把 fopen/fseek/fread/fclose/mmap/munmap 藏起来，对上层只暴露「文件」「映射」。
// llama_file 对齐 llama_mmap.cpp 的 llama_file::impl（FILE* 读元数据、fd=fileno(fp) 供 mmap）；llama_mmap 对齐 llama_mmap::impl（mmap(...)）；去掉 prefetch/numa/madvise 等性能优化。
...
namespace llama
{
    ...
    // 把已 mmap 好的映射「移交」给另一 llama_mmap（如从局部临时移入 llama_model.mmap）；接管后需把源置空，否则双方析构会 munmap 同一地址两次。
