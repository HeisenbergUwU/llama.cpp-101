// gguf.cpp - GGUF 文件裸解析器实现（load + check）：读取对照上游 ggml/src/gguf.cpp 的 gguf_init_from_file，
// 校验对照 src/llama-model-loader.h 的 llama_tensor_weight。省去端序转换/多分片/mmap，只讲「怎么读 + 怎么查」。
...
namespace gguf
{
    ...
    // 游标式顺序读取辅助（与上游 static helper 一致写法）：直接调 llama_file::read_at，这里只补游标推进。
    // 小端序读取一个元素。
    template <typename T>
