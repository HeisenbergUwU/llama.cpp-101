...
namespace ggml
{
    ...
    // ---- 建池 ----（对齐上游 ggml_init）：ggml_context 句柄单独 malloc、不占池子，只是指向池子的管理器；
    // mem_buffer 池子只放对象/tensor；若未提供 mem_buffer，内部 calloc 一块。
    ...
    // ---- 池子切块：末尾追加一块，返回对象头 ----（对齐 ggml_new_object）
    // 布局 [ 对象头 ][ 载荷 ]：offs 指向载荷起点、size 为对齐后载荷大小，下一对象接在载荷之后。
