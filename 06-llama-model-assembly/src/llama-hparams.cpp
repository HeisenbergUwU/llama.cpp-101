// llama-hparams.cpp - 06 章「超参数 HParams」实现：parse_hparams 遍历 gguf_context.kv 按 key 填 HParams。
// 键名 = "<arch>.<subkey>"（plain llama 的 arch 是 "llama"）。只处理 tinybrainbot 需要的字段，缺关键字段即报错。
...
namespace llama
{
    namespace
    {
        ...
        // 读一个非数组、标量数值型 KV：uint32 或 float。05 的 gguf.cpp 把标量统一存成十进制字符串，
