// llama-model.cpp - 05 章「建 llama_model 聚合对象 + 加载权重」实现（对齐上游 llama_model_loader）。
// load_model 5 步：① llama_file 打开一次（fd+size）② llama_mmap 映射整个文件 ③ gguf_load 复用 file 解析元数据取 n_tensors ④ ggml_init 建池（no_alloc，按 tensor 数动态算）⑤ 每 tensor：ggml_new_tensor+set_name + data=mmap.addr()+数据段起点+tensor.offset 零拷贝挂载。
...
namespace llama
{
    ...
    // ---- 析构 ----：池子里只放 ggml_tensor 结构，释放即可；mmap 是 llama_mmap 成员，
    // 在本析构体执行完后由其自己的析构自动 munmap（RAII）。
    ...
    // 用局部变量做完全部工作再一次性写进 llm（中途失败时 llm 保持全空安全态，析构不双重释放）；文件只打开一次供 gguf_load 与 llama_mmap 共用。
    bool load_model(const std::string &path, llama_model &llm, std::string &err)
    {
        ...
        // ---- 06 章：从 KV 解析超参数 ----：直接读 gguf_context.kv（只读元数据，不依赖 tensor 实例化），
        // 结果存进 llm.hparams 供语义组装/前向使用（对齐上游 load_hparams 时机）。
        ...
        // no_alloc=true：池子只装结构。每 tensor 占对象头(32B)+align16(sizeof(ggml_tensor)=160B)=192B，首对象头从池子起点再加 32B，留 1KB 余量。
        ...
        // 数据段起点 = gguf_context.offset（对齐后），每 tensor 再偏 ti.offset；故 data = mmap基址 + gguf_context.offset + ti.offset。
        for (size_t i = 0; i < gguf_context.info.size(); i++)
        {
            const gguf::gguf_tensor_info &ti = gguf_context.info[i];
        ...
        // ---- 全部成功，一次性提交到 llm ----
        // mapping 是局部临时，用 std::move 把映射转移给 llm.mmap（llama_mmap 禁拷贝、可移动，移动后 mapping 的 addr 置空不再 munmap）。
        llm.ctx = ctx;
    ...
    // ---- 06 章：模型语义组装 ----
    // tensor 名（plain llama）：根 = token_embd/output_norm/output.weight；层 = blk.{il}.attn_norm/attn_q/attn_k/attn_v/attn_output/ffn_norm/ffn_gate/ffn_up/ffn_down（后缀 .weight）。先建 name->ggml_tensor* 查找表再按名挂接，缺一即报错。
    bool assemble_model(const llama_model &llm, Model &model, std::string &err)
    {
        ...
        // ---- 根张量 ----
        // output.weight 可选（plain llama 常做权重绑定：lm_head 复用 token_embd，见 src/models/llama.cpp 41-46）。缺失时 model.output 与 model.token_embd 指向同一块。
